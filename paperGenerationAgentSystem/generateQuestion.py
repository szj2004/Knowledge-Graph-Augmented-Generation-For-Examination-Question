...
import json
import time
from gpt import query_chatgpt_model
from save_mysql import get_metarial_all
import random
...
更换模型时改变 query_chatgpt_model算法即可
暂时不用的
...
def generate(data , type, method):
    ...
    res = query_chatgpt_model(prompt)  # 送入LLM
    # print(res)
    return res
...
if __name__ == '__main__':
    start = time.time()
    data=(3440, 742, '1931年', '日本、中国人民', '局部抗战', '九一八事变', '发动事件', '侵华战争')
    method=3
    type=3
    generate(data=data,type=type,method=method)
    end = time.time()
